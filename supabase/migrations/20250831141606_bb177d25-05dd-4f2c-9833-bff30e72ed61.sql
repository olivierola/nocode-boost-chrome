-- Create RPC functions for posts management
CREATE OR REPLACE FUNCTION get_posts_for_project(project_id UUID)
RETURNS TABLE(
  id UUID,
  user_id UUID,
  project_id UUID,
...
BEGIN
  ...
  SELECT 
    ...
    p.updated_at
  FROM public.posts p
  WHERE p.project_id = get_posts_for_project.project_id
  AND EXISTS (
